...
resample_blocks_by_area = function ( x, y, NBoot, lookup_table, block_L, Grid_space, area_or_sites="area", shape=shape,lookup_tablename="lookup_table",lookuptables.folderpath,...) {
  ...
  print(paste("creating lookup table",lookuptables.folderpath,lookup_tablename,"_L",block_L,"_grid_space_",Grid_space,"_sampling_type_",
             "sites","_",shape,".RData"))
    ...
    if (is.na(lookuptables.folderpath) ==FALSE){
      ...
      save(lookup_table,lookup.coords, file=paste0(lookuptables.folderpath,lookup_tablename,"_L",block_L,"_grid_space_",Grid_space,"_sampling_type_",
             "sites","_",shape,".RData"))
    }
  }
  nGrid=nrow(Grid)
